...
with open(csv_file) as f:
    ...
    for row in reader:
        ...
        if approve:
            ...
            row += ['',
                    'Misclassified obvious image (diliberately poor quality)']
        elif failedTrans:
            row += ['',
                    'Failed attention task (selected incorrect transcription)']
        allRows.append(row)
...
print('Total')
print('Correct: {}, guess right: {}, gold: {}/{}'.format(totalCorrect /
      total, totalGuessRight / total, goldRight, totalGold))
...
for workerId, stats in workerStats.items():
    print(
        '{}\t{}\t{}\t{}\tcorrect:{}\tright:{}'.format(
            workerId,
            stats['total'],
            workerStats[workerId]['eReal'],
            workerStats[workerId]['eGen'],
            stats['correct'] /
            stats['total'],
            stats['guessRight'] /
            stats['total']))
    ...
    if correctRatio > 0.9 and stats['total'] > 5:  # final crit
        ...
        accWrkr.append((stats['noGoldRightGuessGen'] +
                        stats['noGoldRightGuessHuman']) /
                       ((stats['noGoldRightGuessGen'] +
                         stats['noGoldRightGuessHuman'] +
                           stats['noGoldWrongGuessGen'] +
                           stats['noGoldWrongGuessHuman'])))
...
filteredTotal = filteredRightGuessHuman + filteredWrongGuessHuman + \
    filteredRightGuessGen + filteredWrongGuessGen
filteredRight = filteredRightGuessHuman + filteredRightGuessGen

print('kept instances:{}/{} {:.3}, workers: {:.3}\twere right: {:.3}'.format(filteredTotal, total,
      filteredTotal / total, filteredWorkers / len(workerStats), filteredRight / filteredTotal))
print('           Guessed: human  computer')
print(
    'Actually human:     {:.3}  {:.3}'.format(
        filteredRightGuessHuman /
        filteredTotal,
        filteredWrongGuessGen /
        filteredTotal))
print(
    'Actually generated: {:.3}  {:.3}'.format(
        filteredWrongGuessHuman /
        filteredTotal,
        filteredRightGuessGen /
        filteredTotal))
