...
widths = []
saveHere = '../data/BEST_images_train'
linenum = 0
...
def display(data):
    ...
    for b in range(batchSize):
        ...
        if draw:
            ...
            cv2.imshow('out/img.png', img.numpy()*255)
            if data['top_and_bottom'] is not None:
                ...
                for h in range(img.size(1)):
                    outline[int((center_line[h] -
                                 top_and_bottom[0, h]).item()), h] = (255, 0, 0)
                    outline[int((center_line[h] +
                                 top_and_bottom[1, h]).item()), h] = (0, 0, 255)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 3:
        ...
        repeat = 1
    data = author_best_dataset.AuthorBESTLinesDataset(dirPath=dirPath, split='train', config={
        'img_height': 64,
        'max_width': 1300,
        'char_file': './data/best_char_set.json',
        'center_pad': False,
        'a_batch_size': 2,
    ...
    dataLoader = torch.utils.data.DataLoader(
        ...
        shuffle=True,
        num_workers=0,
        collate_fn=author_best_dataset.collate)
    ...
    with open(os.path.join(dirPath, 'RIMES_lines_train_gt.txt'), 'w', encoding='utf-8') as out:
        for gt in gts:
            out.write(gt + '\n')
