...
def update_status(name, message):
    try:
        r = requests.get(
            'http://sensei-status.herokuapp.com/sensei-update/{}?message={}'.format(name, message))
...
def main(
    ...
    with torch.no_grad():
        ...
        if index is None:
            ...
            if 'save_style' in config:
                ...
                if lastSlash >= 0:
                    saveStyleValLoc = saveStyleLoc[:lastSlash +
                                                   1] + 'val_' + saveStyleLoc[lastSlash + 1:]
                else:
            ...
            numberOfBatches = min(
                numberOfBatches, max(
                    len(valid_data_loader), len(data_loader)))
            for batch in range(startBatch, numberOfBatches):
                ...
                if valid_data_loader is not None and batch < len(
                    ...
                    print('{} batch index: {}/{}       '.format(validName,
                          batch, len(valid_data_loader)), end='\r')
                    ...
                    ), trainer, metrics, validDir, batch * vBatchSize, toEval=toEval)
                    if type(metricsO) == dict:
                        for typ, typeLists in metricsO.items():
                            if type(typeLists) == dict:
                                for name, lst in typeLists.items():
                        ...
                        val_metrics_sum += metricsO.sum(axis=0) / \
                            metricsO.shape[0]
                    if 'save_spaced' in config:
                ...
                if not test and 'skip_train' not in config:
                    ...
                    if batch < len(data_loader):
                        print(
                            'train batch index: {}/{}        '.format(batch, len(data_loader)), end='\r')
                        ...
                        _, aux = saveFunc(
                            config, instance, trainer, metrics, trainDir, batch * batchSize, toEval=toEval)
                        if 'save_nns' in config:
                            nns += aux[-1]
                        if 'save_spaced' in config:
                        ...
                        if 'save_preds' in config:
                            for b in range(batchSize):
                                try:
                                    to_save.append(
                                        [instance['name'][b], instance['gt'][b], aux['pred_str'][b], aux['cer'][b]])
                                except IndexError:
                                    pass
                ...
                try:
                    if valid_data_loader is not None:
                        for vi in range(batch, len(valid_data_loader)):
                            ...
                            instance = valid_iter.next()
                            metricsO, aux = saveFunc(
                                config, instance, trainer, metrics, toEval=toEval)
                            if type(metricsO) == dict:
                                for typ, typeLists in metricsO.items():
                                    if type(typeLists) == dict:
                                        for name, lst in typeLists.items():
                            ...
                            if 'save_spaced' in config:
                                spaced_val[aux['name'][0]
                                           ] = aux['spaced_label'].cpu()
                            if 'save_style' in config:
                                stylesVal.append(aux['style'].cpu())
                                authorsVal += aux['author']
                                if doIds:
                    ...
                    print(
                        'ERROR: ran out of valid batches early. Expected {} more'.format(
                            len(valid_data_loader) - vi))
            ####
            if valid_data_loader is not None:
            ...
            for typ in val_comb_metrics:
                print(
                    '{} overall mean: {}, std {}'.format(
                        typ, np.mean(
                            val_comb_metrics[typ], axis=0), np.std(
                            val_comb_metrics[typ], axis=0)))
                for name, typeLists in val_metrics_list[typ].items():
                    print(
                        '{} {} mean: {}, std {}'.format(
                            typ, name, np.mean(
                                typeLists, axis=0), np.std(
                                typeLists, axis=0)))
            ...
            if 'save_spaced' in config:
                ...
                if lastSlash >= 0:
                    saveSpacedValLoc = saveSpacedLoc[:lastSlash +
                                                     1] + 'val_' + saveSpacedLoc[lastSlash + 1:]
                else:
