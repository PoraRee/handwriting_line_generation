...
def collate(batch):
    ...
    all_labels = [np.pad(l, ((0, max_len - l.shape[0]),), 'constant')
                  for l in all_labels]
...
class HWDataset(Dataset):
    def __init__(self, dirPath, split, config):
        ...
        # if 'center_pad' in config else True
        self.center = config['center_pad']
                ...
                cv2.imwrite(
                    os.path.join(
                        ...
                        '{}_{}.png'.format(
                            ...
                            line)),
                    img)
        elif self.augmentation is not None and (not isinstance(self.augmentation, str) or 'warp' in self.augmentation):
            #img = augmentation.apply_random_color_rotation(img)
            if isinstance(
                    self.augmentation,
                    str) and "low" in self.augmentation:
                if random.random() > 0.1:
                    img = augmentation.apply_tensmeyer_brightness(img)
                if random.random() > 0.01:
                    img = grid_distortion.warp_image(
                        img, w_mesh_std=0.7, h_mesh_std=0.7)
            else:
                img = augmentation.apply_tensmeyer_brightness(img)
