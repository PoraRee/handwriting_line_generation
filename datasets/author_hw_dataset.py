...
def collate(batch):
    ...
    if 'fg_mask' in batch[0]:
        fg_masks = torch.FloatTensor(
            len(batch) * a_batch_size, 1, dim2, dim3).fill_(0)
    if 'changed_image' in batch[0]:
        changed_batch = torch.FloatTensor(
            len(batch) *
            a_batch_size,
            dim1,
            dim2,
            dim3).fill_(PADDING_CONSTANT)
    top_and_bottom_batch = torch.FloatTensor(
        len(batch) * a_batch_size, 2, dim3).fill_(0)
    center_line_batch = torch.FloatTensor(
        len(batch) * a_batch_size,
        dim3).fill_(
        dim2 / 2)
    labels_batch = torch.IntTensor(
        max_label_len,
        len(batch) *
        a_batch_size).fill_(0)
    if max_spaced_label_len is not None:
        spaced_labels_batch = torch.IntTensor(
            max_spaced_label_len, len(batch) * a_batch_size).fill_(0)
    else:
    ...
    for i in range(len(batch)):
        ...
        input_batch[i * a_batch_size:(i + 1) *
                    a_batch_size, :, :, 0:b_img.shape[3]] = b_img
        ...
        if b_top_and_bottom is not None:
            top_and_bottom_batch[i *
                                 a_batch_size:(i +
                                               1) *
                                 a_batch_size, :, 0:b_img.shape[3]] = b_top_and_bottom
        else:
        ...
        if b_center_line is not None:
            center_line_batch[i *
                              a_batch_size:(i +
                                            1) *
                              a_batch_size, 0:b_img.shape[3]] = b_center_line
        else:
            center_line_batch = None
        labels_batch[0:l.size(0), i * a_batch_size:(i + 1) * a_batch_size] = l
        if max_spaced_label_len is not None:
            sl = batch[i]['spaced_label']
            spaced_labels_batch[0:sl.size(
                0), i * a_batch_size:(i + 1) * a_batch_size] = sl
...
class AuthorHWDataset(Dataset):
    def __init__(self, dirPath, split, config):
        ...
        for page_idx, name in enumerate(set_list):
            lines, author = parseXML(
                os.path.join(
                    dirPath, 'xmls', name + '.xml'))
            self.author_list.add(author)
            if only_author is not None and isinstance(
                    only_author, int) and page_idx == only_author:
                only_author = author
            ...
            if skip_author is not None and author == skip_author:
                continue
            self.max_char_len = max(
                [self.max_char_len] + [len(l[1]) for l in lines])

            authorLines = len(self.authors[author])
            self.authors[author] += [(os.path.join(dirPath,
                                      'forms', name + '.png'),) + l for l in lines]
            ...
            for author, lines in self.lineIndex:
                for line in lines:
                    ...
                    fg_path = os.path.join(
                        self.fg_masks_dir, '{}_{}.png'.format(
                            author, line))
                    if not os.path.exists(fg_path):
                        # read as grayscale, crop line
                        ...
                        if img.shape[0] != self.img_height:
                            ...
                            img = cv2.resize(
                                img, (0, 0), fx=percent, fy=percent, interpolation=cv2.INTER_CUBIC)
                            if img.shape[0] < self.img_height:
                                diff = self.img_height - img.shape[0]
                                img = np.pad(
                                    img, ((diff // 2, diff // 2 + diff %
                                           2), (0, 0)), 'constant', constant_values=255)
        ...
        # config['center_pad'] #if 'center_pad' in config else True
        self.center = False
        ...
        if 'style_loc' in config:
            ...
            for loc in all_style_files:
                ...
                for i in range(len(styles['authors'])):
                    ...
                    by_author_all_ids[styles['authors']
                                      [i]].update(styles['ids'][i])
    ...
    def __getitem__(self, idx):
        if isinstance(
                self.augmentation,
                str) and 'affine' in self.augmentation:
            strech = (self.max_strech * 2) * \
                np.random.random() - self.max_strech + 1
            #self.max_rot_rad = self.max_rot_deg/180 * np.pi
            skew = (self.max_rot_rad * 2) * \
                np.random.random() - self.max_rot_rad
        ...
        if self.triplet == 'hard':
            authors = random.sample(
                self.authors.keys(),
                self.triplet_author_size)
            alines = []
            for author in authors:
                if len(
                        self.authors[author]) >= self.triplet_sample_size * self.batch_size:
                    lines = random.sample(
                        range(len(self.authors[author])), self.triplet_sample_size * self.batch_size)
                else:
                    lines = list(range(len(self.authors[author])))
                    random.shuffle(lines)
                    dif = self.triplet_sample_size * \
                        self.batch_size - len(self.authors[author])
                    lines += lines[:dif]
            ...
            if self.triplet:
                if len(self.authors[author]) <= 2 * self.batch_size:
                    ...
                    unused_lines = set(
                        range(len(self.authors[author]))) - used_lines
        ...
        for author, line in alines:
            ...
            if isinstance(
                self.augmentation,
                str) and 'normalization' in self.augmentation and self.normalized_dir is not None and os.path.exists(
                os.path.join(
                    self.normalized_dir,
                    '{}_{}.png'.format(
                    author,
            ...
            if img.shape[0] != self.img_height:
                ...
                if img.shape[1] * percent > self.max_width:
                    percent = self.max_width / img.shape[1]
                img = cv2.resize(img, (0, 0), fx=percent,
                                 fy=percent, interpolation=cv2.INTER_CUBIC)
                if img.shape[0] < self.img_height:
                    diff = self.img_height - img.shape[0]
                    img = np.pad(
                        img, ((diff // 2, diff // 2 + diff %
            ...
            # we split the processing here so that strech will be adjusted for
            # longest image in author batch
        ...
        for line, gt, img, author in images:
            if self.fg_masks_dir is not None:
                ...
                if fg_mask.shape != img[:, :].shape:
                    print('Error, fg_mask ({}, {}) not the same size as image ({})'.format(
                        fg_path, fg_mask.shape, img[:, :, 0].shape))
            ...
            if isinstance(
                    self.augmentation,
                    str) and 'normalization' in self.augmentation and not readNorm:
                    ...
                    cv2.imwrite(
                        os.path.join(
                            ...
                            '{}_{}.png'.format(
                                ...
                                line)),
                        img)
            if isinstance(self.augmentation,
                          str) and 'affine' in self.augmentation:
                img, fg_mask = augmentation.affine_trans(
                    img, fg_mask, skew, strech)
            elif self.augmentation is not None and (not isinstance(self.augmentation, None) or 'warp' in self.augmentation):
            ...
            if self.include_stroke_aug:
                new_img = augmentation.change_thickness(
                    img, thickness_change, fg_shade, bg_shade, blur_size, noise_sigma)
        ...
        input_batch = np.full(
            (len(batch), dim0, dim1, dim2), PADDING_CONSTANT).astype(
            np.float32)
        if self.fg_masks_dir is not None:
            fg_masks = np.full(
                (len(batch), dim0, dim1, 1), 0).astype(
                np.float32)
        if self.include_stroke_aug:
            changed_batch = np.full(
                (len(batch), dim0, dim1, dim2), PADDING_CONSTANT).astype(
                np.float32)
        for i in range(len(batch)):
            b_img = batch[i]['image']
            ...
            if self.fg_masks_dir is not None:
                fg_masks[i, :, toPad:toPad +
                         b_img.shape[1], 0] = batch[i]['fg_mask']
            if self.include_stroke_aug:
                changed_batch[i, :, toPad:toPad +
                              b_img.shape[1], :] = batch[i]['changed_image']
        ...
        all_labels = [np.pad(l, ((0, max_len - l.shape[0]),),
                             'constant') for l in all_labels]
        all_labels = np.stack(all_labels, axis=1)
        if self.spaced_by_name is not None or self.identity_spaced:
            spaced_labels = [
                np.pad(
                    l, ((0, max_spaced_len - l.shape[0]), (0, 0)), 'constant') for l in spaced_labels]
            ddd = spaced_labels
        ...
        if batch[0]['style'] is not None:
            ...
            styles = None
        mask, top_and_bottom, center_line = makeMask(
            images, self.mask_post, self.mask_random)
        # DEBUG
        # for i in range(5):
