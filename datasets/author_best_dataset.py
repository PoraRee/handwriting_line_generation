...
def collate(batch):
    ...
    for i in range(len(batch)):
        ...
        input_batch[i * a_batch_size:(i + 1) *
                    a_batch_size, :, :, 0:b_img.shape[3]] = 254/255 + b_img/255
        # print(input_batch.shape)
        # cv2.imshow(
        #     'asdf.png', ((1 - input_batch[0].permute(1, 2, 0)) / 2.0).numpy() * 255)
...
class AuthorBESTDataset(Dataset):
                        ...
                        with open(os.path.join(dirPath, file_name, file), 'r', encoding='cp874') as f:
                            for line in f:
                                ...
                                self.max_char_len = max(
                                    self.max_char_len, len(label))
                                self.images.append(
                                    (os.path.join(dirPath, file_name, img_path), label))
    ...
    def __getitem__(self, idx):
        ...
        images = []
        for img_path, gt in [self.images[idx]]:
            author = 0
            if self.no_spaces:
                gt = gt.replace(' ', '')
        ...
        mask, top_and_bottom, center_line = None, None, None
        # DEBUG
        # for i in range(5):
