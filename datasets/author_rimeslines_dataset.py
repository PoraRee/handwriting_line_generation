    ...
    def __init__(self, dirPath, split, config):
        ...
        if only_author is not None:
            raise NotImplementedError(
                'only_author not implemented for RIMES. There arent authors anyways')
        if skip_author is not None:
            raise NotImplementedError(
                'skip_author not implemented for RIMES. There arent authors anyways')
        # minLines=99999
        ...
        for author, lines in self.authors.items():
            self.max_char_len = max(
                self.max_char_len, max([len(l[2]) for l in lines]))
