...
class TextData():
    def __init__(
        ...
        if hardsplit_newline:
            ...
            # This takes a minute on large text, but only needs done once.
            text = re.sub('\\s+', ' ', text)
            self.text = text
            self.words = words
            ...
            if words:
                ...
                for word in words:
                    m = re.match(r'[.,:\'"?!]*', word)
                    if m is None or m.span()[0] != 0 or m.span()[
                            1] < len(word):
    ...
    def getInstance(self):
                ...
                if self.characterBalance:
                    ...
                    while True:
                        ...
                        if flipped and idx >= startIdx:
                            # this char is not in the text set, so we'll just
                            # add it somewhere random
        ...
        if self.char_to_idx is not None:
            ...
            all_labels = [
                np.pad(
                    l, ((0, max_len - l.shape[0]),), 'constant') for l in all_labels]
