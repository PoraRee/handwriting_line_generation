...
def get_style(config, model, instance, gpu=None):
    ...
    if lookup_style:
        ...
        if not style_together:
            ...
            # append all the instances in the batch by the same author together
            ...
            collapsed_image = image.permute(
                ...
                a_batch_size,
                w *
                a_batch_size).permute(
                2,
                0,
                1,
                3)
            collapsed_label = spaced_label.permute(
                1,
                0,
                2).contiguous().view(
                num_class,
...
def main(
    ...
    if addToConfig is not None:
        for add in addToConfig:
            ...
            print(printM)
            if (add[-2] == 'useDetections' or add[-2]
                    == 'useDetect') and value != 'gt':
                addDATASET = True
    ...
    if fromDataset:
        ...
        def get_valid_data_loader():
            ...
            if valid_data_loader is None:
                if not test:
                    data_loader, valid_data_loader = getDataLoader(
                        config, 'train')
                else:
    ...
    # 'mask' in config['model']['generator'] or 'Mask' in config['model']['generator']
    gt_mask = 'create_mask' not in config['model']
                        ...
                        genStep = (
                            (1 -
                             images[i][b].permute(
                                 ...
                                 2,
                                 0)) *
                            127.5).cpu().numpy().astype(
                            np.uint8)
                        path = os.path.join(
                            saveDir, 'gen{}_{}.png'.format(b, i))
                        cv2.imwrite(path, genStep)
            # interpolate randomly selected styles, "v" is VAE
            elif action[0] == 'r' or action[0] == 'v':
                ...
                if action[0] == 'r':
                    ...
                    stylesL = [
                        torch.FloatTensor(
                            1, model.style_dim).normal_() for i in range(num_styles)]
                images = []
                styles = []
                # step=0.05
                for i in range(num_styles - 1):
                    b_im, b_sty = interpolate(model, stylesL[i].to(
                        gpu), stylesL[i + 1].to(gpu), text, char_to_idx, gpu, step)
                    images += b_im
                    styles += b_sty
                b_im, b_sty = interpolate(
                    model, stylesL[-1].to(gpu), stylesL[0].to(gpu), text, char_to_idx, gpu, step)
                images += b_im
                styles += b_sty
                for b in range(images[0].size(0)):
                    for i in range(len(images)):
                    ...
                    torch.save(
                        styles, os.path.join(
                            saveDir, 'styles{}.pth'.format(b)))

            # Just random (interpolated) styles, with option for random text
            elif action[0] == 'R':
                assert(
                    styles is not None and 'perhaps you forgot to set "-s path/to/styles.pkl"?')
                num_inst = int(input('num to gen? '))
                text = input(
                    'text? (enter "RANDOM" or "WIKI" or file path (.txt) for sampled text"): ')
                index_offset = 0
                if len(text) == 0:
                    text = 'The quick brown fox jumps over the lazy dog.'
                    textList = None
                    ...
                    textData = TextData(
                        batch_size=num_inst, max_len=55, textfile=text)
                    textList = textData.getInstance()['gt']
                    text = None
                else:
                    textList = None
                ...
                for i in range(num_inst):
                    ...
                    if charSpec:
                        if gpu is not None:
                            style = (torch.from_numpy(style[0])[None, ...].to(gpu), torch.from_numpy(
                                style[1][None, ...]).to(gpu), torch.from_numpy(style[2][None, ...]).to(gpu))
                        else:
                            style = (torch.from_numpy(style[0])[None, ...], torch.from_numpy(
                                style[1])[None, ...], torch.from_numpy(style[2])[None, ...])
                    else:
                        if gpu is not None:
                ...
                if arguments is None:
                    # whether to interpolate styles, or take the directly from
                    # images
                    interpolateS = input('interpolate? [Y]/N: ')
                ...
                for i in range(num_inst):
                    if not model.vae:
                        ...
                        if interpolateS:
                            authorB = random.choice(get_authors())
                            instance = random.randint(
                                0, len(styles[authorB]) - 1)
                            style2 = styles[authorB][instance]
                ...
                with open(os.path.join(saveDir, 'text.csv'), 'w') as text_out:
                    ...
                    for i in range(num_inst):
                        index = random.randint(
                            0, len(get_test_data_loader()) - 1)
                        instance = test_data_loader.dataset[index]
                        text = instance['gt'][0]
                        textL.append(text)
                        while(True):
                            indexF = random.randint(
                                0, len(test_data_loader) - 1)
                            if indexF != index:
                                break
                        ...
                        to_write.append(
                            [url, re.sub(r'[^\w\s]', '', text), textF, image_name, 'real'])
                    ...
                    for i, (style, text) in enumerate(zip(stylesL, textL)):
                        ...
                        im = (
                            ...
                            127.5).cpu().numpy().astype(
                            np.uint8)
                        image_name = 'sample_{}.png'.format(
                            i + num_inst + start_index)
                        path = os.path.join(saveDir, image_name)
                        cv2.imwrite(path, im)
                        url = 'http://students.cs.byu.edu/~brianld/images/{}'.format(
                            image_name)
                    ...
                    csvwriter.writerow(
                        ['image_url', 'real_text', 'false_text', 'image_name', 'type'])
                    for l in to_write:
                        csvwriter.writerow(l)

            # from given image's style to other image's style.
            elif action[0] == 'f':
                if arguments is None:
                ...
                if image1.shape[0] != img_height:
                    ...
                    image1 = cv2.resize(
                        image1, (0, 0), fx=percent, fy=percent, interpolation=cv2.INTER_CUBIC)
                image1 = image1[..., None]
                ...
                if image2.shape[0] != img_height:
                    ...
                    image2 = cv2.resize(
                        image2, (0, 0), fx=percent, fy=percent, interpolation=cv2.INTER_CUBIC)
                image2 = image2[..., None]
                ...
                style = model.extract_style(torch.stack(
                    (image1[:, :, :min_width], image2[:, :, :min_width]), dim=0), None, 1)
                ...
                images, stylesInter = interpolate(
                    model, style1, style2, text_gen, char_to_idx, gpu)
                ...
                if True:  # new way
                    ...
                    if styles is not None:
                        ...
                        style1 = model.extract_style(image1, label1, a_batch_size)[
                            ::a_batch_size]
                        style2 = model.extract_style(image2, label2, a_batch_size)[
                            ::a_batch_size]
                    images, stylesInter = interpolate(
                        model, style1, style2, text, char_to_idx, gpu)
...
def generate(model, style, text, char_to_idx, gpu):
    ...
    label = torch.from_numpy(label.astype(np.int32))[
        :, None].expand(-1, batch_size).to(gpu).long()
...
def interpolate_horz(model, style, spaced_label):
    ...
    for strechH in np.arange(1, 1.11, 0.01):
        spaced_label = F.interpolate(
            orig_spaced_label,
            scale_factor=strechH,
            mode='linear').permute(
            2,
            0,
            1)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Interactive script to generate images from trained model')
