# from https://github.com/victoresque/pytorch-template, with heavy
# modifications
import os
import math
import json
...
class BaseTrainer:
    ...
    def __init__(
        ...
        self.curriculum = Curriculum(
            config['trainer']['curriculum']) if 'curriculum' in config['trainer'] else None

        self.hwr_frozen = config['trainer']['hwr_frozen'] if 'hwr_frozen' in config['trainer'] else (
            config['model']['hwr_frozen'] if 'hwr_frozen' in config['model'] else False)
        self.style_frozen = config['trainer']['style_frozen'] if 'style_frozen' in config['trainer'] else (
            config['model']['style_frozen'] if 'style_frozen' in config['model'] else False)
        ...
        if config['optimizer_type'] != "none":
            ...
            fix_author_classifer = config['model']['fix_author_classifer'] if 'fix_author_classifer' in config['model'] else (
                config['trainer']['fix_author_classifer'] if 'fix_author_classifer' in config['trainer'] else False)
            for name, param in model.named_parameters():
                if only_params is None or any(
                        [p in name for p in only_params]):
                    goSlow = False
                    freeze = False
            ...
            to_opt = [{'params': main_params}, {
                'params': slow_params, 'lr': config['optimizer']['lr'] * 0.1}]
            self.optimizer = getattr(
                optim, config['optimizer_type'])(
                to_opt, **config['optimizer'])
            if len(discriminator_params) > 0:
                self.optimizer_discriminator = getattr(
                    optim, config['optimizer_type_discriminator'])(
                    discriminator_params, **config['optimizer_discriminator'])
            else:
                self.optimizer_discriminator = None
            if self.curriculum is not None and self.curriculum.need_sep_gen_opt:
                to_opt = [{'params': gen_only_params},
                          {'params': gen_only_slow_params,
                           'lr': config['optimizer']['lr'] * 0.1}]
                self.optimizer_gen_only = getattr(
                    optim, config['optimizer_type'])(
                    to_opt, **config['optimizer'])
            if self.curriculum is not None and self.curriculum.need_sep_style_ex_opt:
                to_opt = [{'params': style_ex_only_params}, {
                    'params': style_ex_only_slow_params, 'lr': config['optimizer']['lr'] * 0.1}]
                self.optimizer_style_ex_only = getattr(
                    optim, config['optimizer_type'])(
                    to_opt, **config['optimizer'])

        self.useLearningSchedule = config['trainer'][
            'use_learning_schedule'] if 'use_learning_schedule' in config['trainer'] else False
        if self.useLearningSchedule == 'LR_test':
            start_lr = 0.000001
            slope = (1 - start_lr) / self.iterations
            def lr_lambda(step_num): return start_lr + slope * step_num
            ...
            def lr_lambda(step_num): return (1 - (1 - min_lr_mul)
                                             * ((step_num - 1) % cycle_size) / (cycle_size - 1))
            self.lr_schedule = torch.optim.lr_scheduler.LambdaLR(
                self.optimizer, lr_lambda)
            ...
            def trueCycle(step_num):
                ...
                if cycle_num % 2 == 0:  # even, rising
                    return ((1 - min_lr_mul) * ((step_num) %
                                                cycle_size) / (cycle_size - 1)) + min_lr_mul
                else:  # odd
                    return (1 - (1 - min_lr_mul) * ((step_num) %
                            cycle_size) / (cycle_size - 1))
            self.lr_schedule = torch.optim.lr_scheduler.LambdaLR(
                self.optimizer, trueCycle)
            ...
            def oneCycle(step_num):
                ...
                if step_num < cycle_size:  # rising
                    return ((1 - low_lr_mul) * ((step_num) %
                                                cycle_size) / (cycle_size - 1)) + low_lr_mul
                elif step_num < cycle_size * 2:  # falling
                    return (1 - (1 - low_lr_mul) * ((step_num) %
                            cycle_size) / (cycle_size - 1))
                else:  # trail off
                    t_step_num = step_num - (2 * cycle_size)
                    return low_lr_mul * (iters_in_trailoff - t_step_num) / \
                        iters_in_trailoff + min_lr_mul * t_step_num / iters_in_trailoff

            self.lr_schedule = torch.optim.lr_scheduler.LambdaLR(
                self.optimizer, oneCycle)
            ...
            def lr_lambda(step_num): return min(
                (step_num + 1)**-0.3, (step_num + 1) * warmup_steps**-1.3)
            self.lr_schedule = torch.optim.lr_scheduler.LambdaLR(
                self.optimizer, lr_lambda)
            ...
            def lr_lambda(step_num): return min((max(0.000001,
                                                     step_num - (warmup_steps - 3)) / 100)**-0.1,
                                                step_num * (1.485 / warmup_steps) + .01)
            # y=((x-(2000-3))/100)^-0.1 and y=x*(1.485/2000)+0.01
            self.lr_schedule = torch.optim.lr_scheduler.LambdaLR(
                self.optimizer, lr_lambda)
        elif self.useLearningSchedule:
            print(
                'Unrecognized learning schedule: {}'.format(
                    self.useLearningSchedule))
            exit()
        ...
        self.checkpoint_dir = os.path.join(
            config['trainer']['save_dir'], self.name)
        ...
        json.dump(
            ...
            indent=4,
            sort_keys=False)
        self.swa = config['trainer']['swa'] if 'swa' in config['trainer'] else (
            config['trainer']['weight_averaging'] if 'weight_averaging' in config['trainer'] else False)
        if self.swa:
            self.swa_model = type(self.model)(config['model'])
            if config['cuda']:
                self.swa_model = self.swa_model.to(self.gpu)
            self.swa_start = config['trainer']['swa_start'] if 'swa_start' in config[
                'trainer'] else config['trainer']['weight_averaging_start']
            self.swa_c_iters = config['trainer']['swa_c_iters'] if 'swa_c_iters' in config[
                'trainer'] else config['trainer']['weight_averaging_c_iters']
    ...
    def train(self):
        ...
        for self.iteration in range(self.start_iteration, self.iterations + 1):
            ...
            if self.swa and self.iteration >= self.swa_start and (
                    self.iterations - self.swa_start) % self.swa_c_iters == 0:
            ...
            for key, value in result.items():
                if key == 'metrics':
                    for i, metric in enumerate(self.metrics):
                        sumLog['avg_' +
                               metric.__name__] += result['metrics'][i]
                else:
            ...
            if (
                self.iteration %
                self.log_step == 0 or self.iteration %
                self.val_step == 0 or self.iteration %
                self.save_step == 0 or (
                    self.save_step_minor is not None and self.iteration %
                    self.save_step_minor)):
            ...
            if self.iteration % self.log_step == 0:
                ...
                for key in sumLog:
                    sumLog[key] = 0
                # we'll do it later if we have a validation pass
                if self.iteration % self.val_step != 0 or self.val_step < 0:
                ...
                if self.swa and self.iteration >= self.swa_start:
                    ...
                    for key, value in val_result.items():
                        if 'metrics' in key:
                            for i, metric in enumerate(self.metrics):
                                log['swa_val_' +
                                    metric.__name__] = val_result[key][i]
                        else:
                            log['swa_' + key] = value
            ...
            if self.iteration % self.save_step == 0:
                ...
                #    print()#clear inplace text
                self.logger.info(
                    'Checkpoint saved for iteration ' + str(self.iteration))
    ...
    def _save_checkpoint(self, iteration, log, save_best=False, minor=False):
        ...
        if not minor:
            filename = os.path.join(
                self.checkpoint_dir,
                'checkpoint-iteration{}.pth' .format(iteration))
        else:
            filename = os.path.join(
                self.checkpoint_dir,
        ...
        if not minor:
            ...
            filename_late = os.path.join(
                self.checkpoint_dir, 'checkpoint-latest.pth')
            try:
                os.remove(filename_late)
            except FileNotFoundError:
                pass
            # os.link(filename,filename_late) #this way checkpoint-latest
            # always does have the latest
            # something is wrong with thel inkgin
        ...
        if save_best:
            os.rename(
                ...
                os.path.join(
                    ...
                    'model_best.pth'))
            self.logger.info(
                "Saved current best: {} ...".format('model_best.pth'))
        else:
    ...
    def _resume_checkpoint(self, resume_path):
        ...
        if ('save_mode' not in self.config or self.config['save_mode']
                == 'state_dict') and 'state_dict' in checkpoint:
            # DEBUG
        ...
        if 'optimizer_gen_only' in checkpoint:
            self.optimizer_gen_only.load_state_dict(
                checkpoint['optimizer_gen_only'])
            if self.with_cuda:
        ...
        if 'optimizer_style_ex_only' in checkpoint:
            self.optimizer_style_ex_only.load_state_dict(
                checkpoint['optimizer_style_ex_only'])
            if self.with_cuda:
        ...
        self.logger.info(
            "Checkpoint '{}' (iteration {}) loaded".format(
                resume_path, self.start_iteration))
