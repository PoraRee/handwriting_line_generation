...
def HWDataset_eval(
    ...
    def __eval_metrics(data, target):
        ...
        for ind in range(output.shape[0]):
            for i, metric in enumerate(metrics):
                acc_metrics[ind,
                            i] += metric(output[ind:ind + 1],
                                         target[ind:ind + 1])
        return acc_metrics

    if isinstance(trainer, HWRWithSynthTrainer):
        pred, losses = trainer.run(instance)
        out = {'pred': pred}
    elif isinstance(trainer, AutoTrainer):
        losses, out = trainer.run_gen(instance, toEval)
    else:  # if type(trainer) is HWWithStyleTrainer:
        if toEval is None:

            pred, recon, losses, style, spaced = trainer.run(
                instance, get_style=True)
            ...
            if spaced is not None:
                out['spaced'] = spaced
        elif isinstance(toEval, list):
            losses, out = trainer.run_gen(
                instance, trainer.curriculum.getEval(), toEval)
        else:
            if toEval == 'spaced':
                justSpaced(
                    trainer.model,
    ...
    if 'pred' in out:
        ...
        for b in range(batchSize):
            ...
            for i in range(len(aligned1[:, b])):
                if aligned1[i, b] != 0 and not (
                        i > 0 and aligned1[i, b] == aligned1[i - 1, b]):
                    a.append(aligned1[i, b].item())
            aligned.append(
                string_utils.label2str_single(
                    a, trainer.idx_to_char, False))

        pred = pred.numpy()
        sum_cer, sum_wer, pred_str, cer = trainer.getCER(
            gt, pred, individual=True)
        out['cer'] = cer
    ...
    if outDir is not None:
                ...
                if 'show_attention' in config:
                    ...
                    colors = (
                        rs.rand(
                            ...
                            trainer.model.style_extractor.keys1.size(1),
                            3) *
                        255).astype(
                        np.uint8)
                    ...
                    for head in range(attn.size(1)):
                        for query in range(attn.size(2)):
                            ...
                            y_pixel_loc = query * \
                                images.shape[2] // attn.size(2)  # + head
                            attn_for[b].append(
                    ...
                    streched_attn = F.interpolate(
                        (attn[0] - minA) / (maxA - minA), size=int(images.shape[3] * batchSize)).cpu()
                for b in range(batchSize):
                    if 'cer_thresh' in config and cer[b] < config['cer_thresh']:
                        continue
                    toColor = False
                    image = (
                        1 - ((1 + np.transpose(images[b][:, :, :], (1, 2, 0))) / 2.0)).copy()
                    if recon is not None:
                        reconstructed = (
                            1 - ((1 + np.transpose(recon[b][:, :, :], (1, 2, 0))) / 2.0)).copy()
                        ...
                        if reconstructed.shape[1] > image.shape[1]:
                            ...
                            image = np.pad(
                                image, ((0, 0), (dif // 2, dif // 2 + dif %
                                                 2), (0, 0)), mode='constant')
                            padded = 'real'
                        elif image.shape[1] > reconstructed.shape[1]:  # pad
                            dif = image.shape[1] - reconstructed.shape[1]
                            reconstructed = np.pad(
                                reconstructed, ((0, 0), (dif // 2, dif // 2 + dif %
                                                         2), (0, 0)), mode='constant')
                            padded = 'gen'
                        border = np.zeros((2, image.shape[1], image.shape[2]))
                        bigPic = np.concatenate(
                            (image, border, reconstructed), axis=0)
                        ...
                        if bigPic.shape[2] == 1 and True:
                            ...
                            if padReal != 0:
                                ...
                                bigPic[image.shape[0],
                                       padReal:-padReal, 1] = 255
                    ...
                    if 'show_attention' in config:
                        ...
                        if 'full' in config['show_attention']:
                            attnImage = np.zeros(
                                (attn.size(1) * attn.size(2), bigPic.shape[1], 3))
                            for head in range(attn.size(1)):
                                for query in range(attn.size(2)):
                                    # query*images.shape[2]//attn.size(2) #+
                                    # head
                                    y_pixel_loc = head + attn.size(1) * query
                                    x_start = int(b * image.shape[1])
                                    x_end = int((b + 1) * image.shape[1])
                                    if head < 3:
                                        attnImage[y_pixel_loc,
                                                  0:image.shape[1],
                                                  head] = streched_attn[head,
                                                                        query,
                                                                        x_start:x_end].numpy()
                                    else:
                                        attnImage[y_pixel_loc,
                                                  0:image.shape[1],
                                                  head % 3] = streched_attn[head,
                                                                            query,
                                                                            x_start:x_end].numpy()
                                        attnImage[y_pixel_loc,
                                                  0:image.shape[1],
                                                  (head + 1) % 3] = streched_attn[head,
                                                                                  query,
                                                                                  x_start:x_end].numpy()
        ...
        if 'gen' in out or 'gen_img' in out:
            ...
            for b in range(batchSize):
                generated = (
                    1 - ((1 + np.transpose(gen[b][:, :, :], (1, 2, 0))) / 2.0)).copy() * 255
...
def justSpacing(model, instance, gpu):
    ...
    if model.spacing_pred is None:
        ...
        recon, style = model.autoencode(
            image, label, mask, a_batch_size, center_line=center_line)
    batch_size = label.size(1)
