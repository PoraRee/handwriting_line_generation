...
class HWWithStyleTrainer(BaseTrainer):
    ...
    def __init__(self, model, loss, metrics, resume, config,
        ...
        self.lossWeights = config['loss_weights'] if 'loss_weights' in config else {
            "auto": 1, "recog": 1}
        ...
        text_data_batch_size = config['trainer']['text_data_batch_size'] if 'text_data_batch_size' in config[
            'trainer'] else self.config['data_loader']['batch_size']
        ...
        if data_loader is not None:
            if 'text_data' in config['trainer']:
                ...
                self.text_data = TextData(
                    config['trainer']['text_data'],
                    config['data_loader']['char_file'],
                    text_data_batch_size,
                    max_len=text_data_max_len,
                    words=text_words,
                    characterBalance=characterBalance) if 'text_data' in config['trainer'] else None

        # balance the CTC loss with others as in
        # https://arxiv.org/pdf/1903.00277.pdf
        self.balance_loss = config['trainer']['balance_loss'] if 'balance_loss' in config['trainer'] else False
        if self.balance_loss:
            ...
            if self.balance_loss.startswith('sign_preserve_x'):
                self.balance_x = float(
                    self.balance_loss[self.balance_loss.find('x') + 1:])
            # this will hold the gradients for previous training steps if
            # "no-step" is specified
            self.saved_grads = []

        self.style_detach = config['trainer']['detach_style'] if 'detach_style' in config['trainer'] else (
            config['trainer']['style_detach'] if 'style_detach' in config['trainer'] else False)

        # setup for keeping history of style vectors
        self.interpolate_gen_styles = config['trainer'][
            'interpolate_gen_styles'] if 'interpolate_gen_styles' in config['trainer'] else False
        if type(
                self.interpolate_gen_styles) is str and self.interpolate_gen_styles[:6] == 'extra-':
            self.interpolate_gen_styles_low = - \
                float(self.interpolate_gen_styles[6:])
        ...
        self.sometimes_interpolate = config['trainer'][
            'sometimes_interpolate'] if 'sometimes_interpolate' in config['trainer'] else False
        ...
        if 'encoder_weights' in config['trainer']:
            snapshot = torch.load(
                config['trainer']['encoder_weights'],
                map_location='cpu')
            encoder_state_dict = {}
            ...
            if 'encoder_type' not in config['trainer'] or config['trainer']['encoder_type'] == 'normal':
                ...
                raise NotImplementedError(
                    'Unknown encoder type: {}'.format(
                        config['trainer']['encoder_type']))
        ...
        self.serperate_print_every = config['trainer'][
            'serperate_print_every'] if 'serperate_print_every' in config['trainer'] else 2500
    ...
    def _train_iteration(self, iteration):
        ...
        if self.curriculum and all(
                [l[:3] == 'gen' or l == 'no-step' for l in lesson]) and self.text_data is not None:
            ...
            try:
                ...
                if 'refresh_data' in dir(self.data_loader.dataset):
                    self.data_loader.dataset.refresh_data(
                        None, None, self.logged)
        ...
        if self.curriculum:
            ...
            if (self.iter_to_print <= 0 or self.print_next_gen) and 'gen' in lesson:
                losses, got = self.run_gen(
                    instance, lesson, get=['gen', 'disc'])
                self.print_images(
                    got['gen'],
                    instance['gt'],
                    got['disc'],
    ...
    def _valid_epoch(self):
        ...
        with torch.no_grad():
            ...
            for batch_idx, instance in enumerate(self.valid_data_loader):
                if not self.logged:
                    print('validate: {}/{}'.format(batch_idx,
                          len(self.valid_data_loader)), end='\r')
                if self.curriculum:
    ...
    def onehot(self, label):
        label_onehot = torch.zeros(
            label.size(0), label.size(1), self.num_class)
        label_onehot.scatter_(
            2, label.cpu().view(
                label.size(0), label.size(1), 1), 1)
    ...
    def run_hwr(self, instance):
        ...
        recogLoss = self.loss['recog'](
            pred, label.permute(
                1, 0), pred_size, label_lengths)
    ...
    def run_gen(self, instance, lesson, get=[]):
        ...
        # CACHED spaced_label
        need_spaced_label = any(
            [x in lesson for x in ['count', 'auto', 'disc']])
        if need_spaced_label:
            if instance['spaced_label'] is not None:
                self.model.spaced_label = self.model.onehot(
                    instance['spaced_label']).to(label.device)
        ...
        if 'auto' in lesson:
            ...
            if 'eval' not in lesson or 'recon' in get:
                recon, style = self.model.autoencode(
                    image, label, a_batch_size)
                if 'styleReg' in self.loss:
                    losses['styleRegLoss'] = self.loss['styleReg'](
                        style, **self.loss_params['styleReg'])
        ...
        if 'gen' in lesson or 'disc' in lesson or 'gen' in get:
            ...
            if 'eval' not in lesson and 'valid' not in lesson or not self.interpolate_gen_styles:
                ...
                # During evaluation, we don't have a bank of previous styles,
                # so we use the previous and current
                ...
                for i in range(batch_size // a_batch_size):
                    ...
                    b_1 = (
                        (i + 1) %
                        (batch_size // a_batch_size)) * a_batch_size
                    style_gen[b:b + a_batch_size] = 0.5 * style[b_1:b_1 +
                                                                a_batch_size] + 0.5 * style[b:b + a_batch_size]
            ...
            if 'eval' not in lesson and label.size(0) > self.text_data.max_len:
                ...
                for b in range(batch_size):
                    label_lengths[b] = min(
                        label_lengths[b], self.text_data.max_len)
        ...
        if 'auto' in lesson and 'auto' in self.loss and 'eval' not in lesson:
            ...
            if recon.size(3) > image.size(3):
                ...
                if toPad > 50:
                    print(
                        'WARNING image {} bigger than recon {}'.format(
                            image.size(3), recon.size(3)))
            ...
            autoLoss = self.loss['auto'](
                recon_autol, image_autol, **self.loss_params['auto'])
        ...
        if 'count' in lesson and 'count' in self.loss and 'eval' not in lesson:
                ...
                if '$UNKOWN$' in instance['gt']:
                    ...
                    for b in range(batch_size):
                        if instance['gt'][b] == '$UNKOWN$':
                            ...
                            pred_str, raw_pred = string_utils.naive_decode(
                                logits.detach().cpu().numpy())
                            pred_str = string_utils.label2str_single(
                                pred_str, self.idx_to_char, False)
                            if len(pred_str) > 0:
                                psuedo_gt.append(pred_str)
                                psuedo_labels.append(
                                    torch.from_numpy(
                                        string_utils.str2label_single(
                                            pred_str,
                                            self.data_loader.dataset.char_to_idx).astype(
                                            np.int32)))
            ...
            if self.model.count_duplicates:
                gt_counts = torch.FloatTensor(
                    label.size(0), batch_size, 2).fill_(0)
                for b in range(batch_size):
                    c = 0  # blanks
                    d = 0  # repeated char
                    pos = 0
                ...
                gt_counts = torch.FloatTensor(
                    label.size(0), batch_size, 1).fill_(0)
                for b in range(batch_size):
                    c = 0
                    pos = 0
                    last = -1
            ...
            losses['countLoss'] = self.loss['count'](
                self.model.counts, gt_counts, **self.loss_params['count'])
            assert not torch.isnan(
                losses['countLoss']) and not torch.isinf(
                losses['countLoss'])
        ...
        if 'auto' in lesson and 'perceptual' in self.loss and 'eval' not in lesson:
            ...
            if image.size(3) > recon.size(3):
                ...
                if diff > 1:
                    print(
                        'Warning, different sizes between image {} and recon {}'.format(
                            image.size(), recon.size()))
            ...
            orig_features, recon_features = zip(
                *[torch.chunk(b, 2, dim=0) for b in both_f])

            perceptual_loss = 0
            for r_f, o_f in zip(recon_features, orig_features):
                perceptual_loss += self.loss['perceptual'](
                    r_f, o_f, **self.loss_params['perceptual'])
        ...
        if 'auto' in lesson and 'reconRecog' in self.loss and 'eval' not in lesson:
            ...
            recon_pred_size = torch.IntTensor(
                [recon_pred.size(0)] * batch_size)
            recon_recogLoss = self.loss['reconRecog'](
                recon_pred, label.permute(
                    1, 0), recon_pred_size, label_lengths)
        ...
        if 'gen' in lesson and 'genRecog' in self.loss and 'eval' not in lesson:
            ...
            gen_recogLoss = self.loss['genRecog'](
                gen_pred, label.permute(
                    1, 0), gen_pred_size, label_lengths)
        ...
        if 'disc' in lesson:
            ...
            discriminator_pred = self.model.discriminator(
                torch.cat((image, fake.detach()), dim=0))
            # hinge loss
            disc_loss = 0
            for i in range(len(discriminator_pred)
                ...
                discriminator_pred_on_fake = discriminator_pred[i][image.size(
                    0):]
                disc_loss += F.relu(1.0 - discriminator_pred_on_real).mean() + \
                    F.relu(1.0 + discriminator_pred_on_fake).mean()
            ...
            for name in get:
                if name == 'recon':
                    ...
                    if gen_image is not None:
                        ...
                        print(
                            'ERROR, gen_image is None, lesson: {}'.format(lesson))
                        #got[name] = None
                elif name == 'pred':
                    ...
                    if self.model.spaced_label is None:
                        if self.model.pred is None:
                            self.model.pred = self.model.hwr(image, None)
                        self.model.spaced_label = correct_pred(
                            self.model.pred, label)
    ...
    def correct_pred(self, pred, label):
        ...
        label_with_blanks = torch.LongTensor(
            label.size(0) * 2 + 1, label.size(1)).zero_()
        ...
        new_labels = [F.pad(l, (0, maxlen - l.size(0)), value=0)
                      for l in new_labels]
    ...
    def get_style_gen(self, batch_size, device):
        if (self.interpolate_gen_styles and (len(self.prev_styles) > 0 or len(self.prev_g_styles) > 0)) and (
                not self.sometimes_interpolate or self.interpolate_freq > random.random()):
            indexes = np.random.randint(
                0, len(self.prev_styles), (batch_size, 2))
            mix = np.random.uniform(
                self.interpolate_gen_styles_low,
                self.interpolate_gen_styles_high,
                batch_size)
            new_styles = []
            for b in range(batch_size):
                new_style = self.prev_styles[indexes[b, 0]] * mix[b] + \
                    self.prev_styles[indexes[b, 1]] * (1 - mix[b])
                new_styles.append(new_style)
            style_gen = torch.stack(new_styles, dim=0).to(device)
        elif self.model.vae or self.model.style_from_normal is None:
            style_gen = torch.FloatTensor(
                batch_size, self.model.style_dim).normal_().to(device)
        else:
            sample = torch.FloatTensor(
                batch_size, self.model.style_dim // 2).normal_()
    ...
    def print_images(self, images, text, disc=None, typ='gen', gtImages=None):
        if self.print_dir is not None:
            ...
            if self.iteration - \
                    self.last_print_images[typ] >= self.serperate_print_every:
