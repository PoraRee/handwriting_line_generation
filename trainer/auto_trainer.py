...
class AutoTrainer(BaseTrainer):
    ...
    def _train_iteration(self, iteration):
        ...
        if self.curriculum and all(
                [l[:3] == 'gen' for l in lesson]) and self.text_data is not None:
    ...
    def run_gen(self, instance, get=[]):
        ...
        if image.size(3) % 8 > 0:
            ...
            image = F.pad(
                image, (toPad // 2, toPad // 2 + toPad %
                        2), value=PADDING_CONSTANT)
        if 'recog' in self.loss:
