...
def warp_image(img, random_state=None, **kwargs):
    ...
    # Get control points
    source = np.mgrid[0:h +
                      h_mesh_interval:h_mesh_interval, 0:w +
                      w_mesh_interval:w_mesh_interval]
    ...
    destination[:, 0] = destination[:, 0] + \
        random_state.normal(0.0, h_mesh_std, size=source_shape)
    destination[:, 1] = destination[:, 1] + \
        random_state.normal(0.0, w_mesh_std, size=source_shape)
    ...
    grid_z = griddata(
        ...
        (grid_x,
         grid_y),
        method=interpolation_method).astype(
        np.float32)
    map_x = grid_z[:, :, 1]
    map_y = grid_z[:, :, 0]
    meanV = img.mean()
