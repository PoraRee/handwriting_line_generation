...
def makeMask(image, post=[], random=False):
    ...
    if len(post) > 0 and post[0] == 'true':
        ...
        pool = torch.nn.MaxPool2d(
            (v_kernel, h_kernel), stride=1, padding=(
                v_padding, h_padding))
        blur_kernel = 3
        blur_padding = blur_kernel // 2
        blur = torch.nn.AvgPool2d(
            (blur_kernel, blur_kernel), stride=1, padding=(
    ...
    for i in range(batch_size):
        ...
        result = np.minimum(
            np.minimum(
                cummax_img0, cummax_img1), np.minimum(
                cummax_img2, cummax_img3))
    ...
    for task in post:
        if task == 'thresh':
            ...
            errode_weights = torch.FloatTensor(
                1, 1, morph_kernel_errode, morph_kernel_errode).fill_(1)
            # one-pad to allow edge pixels to be kept
            #out = F.pad(out,(morph_padding,morph_padding,morph_padding,morph_padding),value=1)
            # ,padding=morph_padding)
            out = F.conv2d(out.float(), errode_weights, stride=1)
            out = out >= (morph_kernel_errode**2)
        elif task == 'errodeCircle':
            errode_weights = torch.FloatTensor(
                1, 1, morph_kernel_errode, morph_kernel_errode)
            r = morph_kernel_errode // 2
            for x in range(morph_kernel_errode):
                for y in range(morph_kernel_errode):
                    errode_weights[0, 0, y, x] = float(
                        ((y - r)**2 + (x - r)**2) <= (r**2))
            out = F.conv2d(
                out.float(),
                errode_weights,
                stride=1,
                padding=morph_padding_errode)  # ,padding=morph_padding)
            out = out >= errode_weights.sum()
        elif task == 'dilate':
            dilate_weights = torch.FloatTensor(
                1, 1, morph_kernel_dilate, morph_kernel_dilate).fill_(1)
            out = F.conv_transpose2d(
                out.float(),
                dilate_weights,
                stride=1)  # ,padding=morph_padding)
            out = out > 0.1
        elif task == 'dilateCircle':
            dilate_weights = torch.FloatTensor(
                1, 1, morph_kernel_dilate, morph_kernel_dilate)
            r = morph_kernel_dilate // 2
            for x in range(morph_kernel_dilate):
                for y in range(morph_kernel_dilate):
                    dilate_weights[0, 0, y, x] = float(
                        ((y - r)**2 + (x - r)**2) <= (r**2))
            out = F.conv_transpose2d(
                out.float(),
                dilate_weights,
                stride=1,
                padding=morph_padding_dilate)  # ,padding=morph_padding)
            out = out > 0.1
        elif task == 'distance':
            out = out.numpy()
            height = out.shape[2]
            ...
            for b in range(batch_size):
                ...
                for x_start in range(0, width - window, window // 2):
                    ...
                    if on > 0:
                        med_x = (
                            x_indexes * out[b, 0, :, x_start:x_start + window]).sum() / on + x_start
                        med_y = (
                            y_indexes * out[b, 0, :, x_start:x_start + window]).sum() / on
                ...
                med_y = sum_y / count
                if len(medians) > 1:
                    slope = (medians[1][0] - medians[0][0]) / \
                        (medians[1][1] - medians[0][1])
                    distance = -medians[0][1]
                    front_point = [
                        (med_y + medians[0][0] + slope * distance) / 2, 0]
                    slope = (medians[-1][0] - medians[-2][0]) / \
                        (medians[-1][1] - medians[-2][1])
                    distance = width - 1 - medians[-1][1]
                    last_point = [(med_y + medians[-1][0] +
                                   slope * distance) / 2, width - 1]
                ...
                for i in range(1, len(medians)):
                    rr, cc = draw.line(int(
                        medians[i - 1][0]), int(medians[i - 1][1]), int(medians[i][0]), int(medians[i][1]))
                    line_im[rr, cc] = 0
            ...
            raise NotImplementedError(
                'unknown makeMask post operation: {}'.format(task))
    ...
    if len(post) > 0:
        ...
        ranges = (torch.arange(height) +
                  1)[None, None, ..., None].expand(batch_size, -1, -1, width)
        ...
        top_and_bottom = torch.cat(
            (centerV - top.float(), bottom.float() - centerV), dim=1)
...
def getCenterValue(mask):
    ...
    for b in range(batch_size):
        ...
        for x_start in range(0, width - window, window // 2):
            ...
            if on > 0:
                med_x = (
                    x_indexes * mask[b, 0, :, x_start:x_start + window]).sum() / on + x_start
                med_y = (y_indexes * mask[b, 0, :,
                         x_start:x_start + window]).sum() / on
                medians.append((med_y, med_x))  # x_start+window//2))
                sum_x += med_x
                sum_y += med_y
                # assert(med_y<height)
