    ...
    for i in range(num_lines):
        ...
        for x, y in zip(*sk_line(startX, startY, endX, endY)):
            if x >= 0 and x < img.shape[1] and y >= 0 and y < img.shape[0]:
                ...
                # print('w [{}:{}, {}:{}
                line_img[yBlockStart:yBlockEnd,
                         xBlockStart:xBlockEnd] = darkness
...
def mmd_crop(img):
    if random.random() < 0.1 or img.shape[1] <= img.shape[0]:
        ...
        # in the mmd dataset, we have a lot of excess space on the ends, I'd
        # like to remove that
...
def bad_crop(img):
    if random.random() < 0.1:
        ...
        if random.random() < 0.6:
            ...
            img = np.pad(img, ((cropTop, -cropBot), (0, 0)),
                         mode='constant', constant_values=img.mean())
        new_h = img.shape[0]
        if new_h == h:
