    ...
    for angle in np.arange(-angle_range, angle_range + 0.001, angle_step):
        ...
        for x in range(0, img.shape[1], x_step):
            ...
            if x_end < img.shape[1]:
                ...
                xL, yL = np.linspace(
                    x, x_end, length), np.linspace(
                    0, img.shape[0] - 1, length)

                v = v_img[yL.astype(np.int),
                          xL.astype(np.int)].sum() / img.shape[0]
                values.append(v)
        var = np.var(values)
...
def skeletonize(img):
    ...
    ret, th = cv2.threshold(
        255 - img, 0, 1, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
    ...
    out = F.conv_transpose2d(
        ...
        dilate_weights,
        stride=1,
        padding=1)  # ,padding=morph_padding)
