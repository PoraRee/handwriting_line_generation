...
def convertBBs(bbs, rotate, numClasses):
    if bbs.shape[1] == 0:
        return None
    # 5 params, 8 points (used in loss), n classes
    new_bbs = np.empty((1, bbs.shape[1], 5 + 8 + numClasses), dtype=np.float32)
    ...
    # projection of half-left edge onto transpose horz run
    hl = ((tlX - lX) * -(rY - lY) + (tlY - lY) * (rX - lX)) / d
    # projection of half-right edge onto transpose horz run
    hr = ((brX - rX) * -(lY - rY) + (brY - rY) * (lX - rX)) / d
    h = (hl + hr) / 2.0
...
# This annotation corrects assumptions made during GTing, modifies the
# annotations for the current parameterization, and slightly changes the
# format
def fixAnnotations(this, annotations):
    ...
    # enumerations paired with the left row of a chained row need to be paired
    # with the right
    pairsToRemove = []
    pairsToAdd = []
    for bb in annotations['textBBs']:
            ...
            for pair in annotations['pairs']:
                if bb['id'] in pair:
                    ...
                    if otherBB['type'] == 'fieldRow':
                        if avg_x(bb) > left_x(otherBB) and avg_x(
                                bb) < right_x(otherBB):
    ...
    # skipped link between col and enumeration when enumeration is between col
    # header and col
    for pair in annotations['pairs']:
        notNum = num = None
        if pair[0] in annotations['byId'] and annotations['byId'][pair[0]
                                                                  ]['type'] == 'textNumber':
            ...
            for pair2 in annotations['pairs']:
                if notNum['id'] in pair2:
                    if notNum['id'] == pair2[0]:
                        ...
                        otherId = pair2[0]
                    if annotations['byId'][otherId]['type'] == 'fieldCol' and avg_y(
                            annotations['byId'][otherId]) > avg_y(annotations['byId'][num['id']]):
    ...
    for pair in toAdd:
        assert(len(pair) == 2)
        if pair not in annotations['pairs'] and [
                pair[1], pair[0]] not in annotations['pairs']:
...
def getBBWithPoints(useBBs, s, useBlankClass=False, usePairedClass=False):
    ...
    if usePairedClass:
        numClasses += 1
    # 2x4 corners, 2x4 cross-points, 2 classes
    bbs = np.empty((1, len(useBBs), 8 + 8 + numClasses), dtype=np.float32)
    for j, bb in enumerate(useBBs):
        ...
        if useBlankClass:
            if bb['type'] == 'detectorPrediction':
                ...
                blank = (bb['isBlank'] == 'blank' or bb['isBlank']
                         == 3) if 'isBlank' in bb else False
...
def getStartEndGT(useBBs, s, useBlankClass=False):
    ...
    if useBlankClass:
        ...
        numClasses = 2
    start_gt = np.empty((1, len(useBBs), 4 + numClasses),
                        dtype=np.float32)  # x,y,r,h, x classes
    end_gt = np.empty((1, len(useBBs), 4 + numClasses),
                      dtype=np.float32)  # x,y,r,h, x classes
    j = 0
    for bb in useBBs:
        tlX = bb['poly_points'][0][0]
...
def getResponseBBIdList_(this, queryId, annotations):
    responseBBList = []
    # done already +annotations['samePairs']:
    for pair in annotations['pairs']:
        if queryId in pair:
            if pair[0] == queryId:
                otherId = pair[1]
                ...
                otherId = pair[0]
            if otherId in annotations['byId'] and (
                not this.onlyFormStuff or (
                    'paired' in bb and bb['paired'])):
