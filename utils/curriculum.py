                ...
                for lesson in lessons:
                    ...
                    for a in lesson:
                        if isinstance(a, str):
                            ...
                            new_lesson.append(a)
                            # as GAN losses aren't too informative...
                            if 'gen' not in a and 'disc' not in a and a != 'split-style' and 'triplet' not in a:
                                self.valid.add(a)
                            ...
                            raise ValueError(
                                'unknown thing in lessons: {}'.format(a))
                    for i in range(dup):
                        lessons_expanded.append(new_lesson)
                self.lessons.append((int(iteration), lessons_expanded))

        # self.lessons.sort(lambda a,b: b[0]-a[0]) #reverse sort based on
        # iteration
