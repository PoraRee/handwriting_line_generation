    ...
    for line in root.findall('./handwritten-part/line'):
        ...
        for word in line.findall('word'):
            ...
            for cmp in word.findall('cmp'):
                ...
                w_minY = min(w_minY, y)
            words.append(
                ([w_minY, w_maxY + 1, w_minX, w_maxX + 1], w_trans, w_id))
...
if __name__ == "__main__":
    ...
    print('num words train: {},  valid: {},  test:  {}'.format(
        len(wordsTrain), len(wordsValid), len(wordsTest)))

    # with open(lineOut,'w') as f:
    #    json.dump({'train':linesTrain,'valid':linesValid,'test':linesTest},f)
    with open(wordOut, 'w') as f:
        json.dump(
            {'train': wordsTrain, 'valid': wordsValid, 'test': wordsTest}, f)
