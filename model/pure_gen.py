...
class SpacedGenerator(nn.Module):
    def __init__(
        ...
        layers = [PixelNorm()]
        drop = emb_dropout if isinstance(emb_dropout, float) else 0.5
    ...
    def forward(
            ...
            content,
            style,
            return_intermediate=False):  # , noise=None):
        content = content.permute(1, 2, 0)  # swap [T,b,cls] to [b,cls,T]
        content = content.view(
            content.size(0),
            content.size(1),
            1,
            content.size(2))  # now [b,cls,H,W]

        style = self.style_emb(style)
        if self.append_style:
            content = torch.cat(
                (content, style[:, :, None, None].expand(-1, -1, 1, content.size(3))), dim=1)
...
class Blur(nn.Module):
    def __init__(self, channel):
        ...
        self.register_buffer(
            'weight_flip', weight_flip.repeat(
                channel, 1, 1, 1))
...
class PixelNorm(nn.Module):
    ...
    def forward(self, input):
        return input / torch.sqrt(torch.mean(input ** 2,
                                             dim=1 if len(input.size()) > 1 else 0,
                                             keepdim=True) + 1e-8)
