...
class HWWithStyle(BaseModel):
    def __init__(self, config):
        ...
        if 'char' in style_type:
            ...
            # config['style_final_g_spacing'] if 'style_final_g_spacing' in config else 1
            num_final_g_spacing_style = 1
            # config['style_char_layers'] if 'style_char_layers' in config else 1
            num_char_fc = 1
        ...
        if 'CRNN' in hwr_type:
            ...
            if 'small' in hwr_type:
                self.hwr = SmallCRNN(
                    num_class, norm=norm, use_softmax=use_softmax)
            else:
        ...
        if 'pretrained_hwr' in config and config['pretrained_hwr'] is not None:
            if os.path.exists(config['pretrained_hwr']):
                snapshot = torch.load(
                    config['pretrained_hwr'], map_location='cpu')
                hwr_state_dict = {}
                ...
                print(
                    'Could not open pretrained HWR weights at ' +
                    config['pretrained_hwr'])
                exit(1)
        ...
        if 'generator' in config and config['generator'] == 'none':
            ...
            self.generator = SpacedGenerator(
                num_class,
                style_dim,
                g_dim,
                n_style_trans=n_style_trans,
                emb_dropout=emb_dropout,
                append_style=append_style,
                small=small)
        else:
            raise NotImplementedError(
                'unknown generator: ' + config['generator'])
        ...
        if 'discriminator' in config and config['discriminator'] is not None:
            ...
            self.discriminator = DiscriminatorAP(
                dim, use_low=use_low, use_med=use_med, small=small)

        if 'spacer' in config and config['spacer']:
            self.count_duplicates = isinstance(
                config['spacer'], str) and 'duplicate' in config['spacer']
    ...
    def forward(self, label, label_lengths, style, spaced=None):
        ...
        if spaced is None:
            ...
            spaced, padded = self.insert_spaces(
                label, label_lengths, self.counts)
            ...
            if spaced.size(0) > self.max_gen_length:
                ...
                for x in range(
                        spaced.size(0) - 1, 0, -1):  # iterate backwards till we meet non-blank
                    if (chars[x] > 0).any():
                        break
                # "+2" to pad out a couple blanks
            ...
            if spaced.size(0) > self.max_gen_length:
                ...
                chars = spaced.argmax(2)
                for x in range(
                        spaced.size(0)):  # iterate forwards till we meet non-blank
                    if (chars[x] > 0).any():
                        break
                # "-2" to pad out a couple blanks
                toRemove = max(min(diff, x - 2), 0)
                if toRemove > 0:
                    spaced = spaced[toRemove:]
    ...
    def autoencode(
            ...
            label,
            a_batch_size=None,
            stop_grad_extractor=False):
        style = self.extract_style(image, label, a_batch_size)
        if stop_grad_extractor:
            # This is used when we use the auto-style loss, as wer're using the
            # extractor result as the target
            style = style.detach()
    ...
    def extract_style(self, image, label, a_batch_size=None):
        ...
        collapsed_image = image.permute(
            ...
            1,
            3)
        collapsed_label = spaced.permute(
            1,
            0,
            2).contiguous().view(
            self.num_class,
        ...
        style = torch.cat([style[i:i + 1].repeat(a_batch_size, 1)
                          for i in range(style.size(0))], dim=0)
        return style
    ...
    def insert_spaces(self, label, label_lengths, counts):
        ...
        for b in range(batch_size):
            ...
            for i in range(label_lengths[b]):
                count = round(np.random.normal(
                    counts[i, b, 0].item(), self.count_std))
                if self.count_duplicates:
                    duplicates = round(np.random.normal(
                        counts[i, b, 1].item(), self.dup_std))
                else:
                    duplicates = 1
    ...
    def onehot(self, label):  # tensorized version
        ...
        label_onehot_v = label_onehot.view(
            label.size(0) * label.size(1), self.num_class)
        label_onehot_v[torch.arange(0,
    ...
    def space_style(self, spaced, style, device=None):
        ...
        style = torch.FloatTensor(
            spaced.size(0),
            batch_size,
            self.char_style_dim).to(device)
        text_chars = spaced.argmax(dim=2)
        # add temporal dim for broadcast
        spacing_style = spacing_style[None, :, :]
        # Put character styles in appropriate places. Fill in rest with
        # projected global style
        for b in range(batch_size):
            lastChar = -1
            for x in range(0, text_chars.size(0)):
                if text_chars[x, b] != 0:
                    ...
                    style[lastChar +
                          1:x, b, :] = spacing_style[:, b]  # broadcast
                    lastChar = x
