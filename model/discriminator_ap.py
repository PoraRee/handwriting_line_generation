...
class DiscriminatorAP(nn.Module):
    ...
    def __init__(self, dim=64, use_low=False, use_med=True, small=False):
        ...
        self.in_conv = nn.Sequential(
            ...
            # Experiments with other GAN showed better results not using
            # spectral on first layer
        ...
        convs1 = [
            SpectralNorm(
                nn.Conv2d(
                    dim, dim, 3, stride=1, padding=(
                        convs1_pad_v, 1))), nn.LeakyReLU(
                leak, True)]
        if not small:
    ...
    def forward(self, x, return_features=False):
        ...
        if self.use_low:
            ...
            # return torch.cat(
            # (pM.view(batch_size,-1),pL.view(batch_size,-1)), dim=1)#.view(-1)
        else:
